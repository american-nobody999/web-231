...
//innerHTML can cause security issues when the inserted value contains untrusted user input, because the browser parses it as HTML and potentially executes injected scripts. This is called cross-site scripting, or XSS.

//For this project, the current use is low risk because:

//htmlCode is built from hard-coded captions
//The image filenames are generated from numeric loop values.
//No user-entered or server-provided content is included in htmlCode
//Risk will increase if these conditions change, such as if user input or server-provided content is included in htmlCode.
// if captions[i] came from a form, URL, database, or API. In that case, use DOM methods such as createElement() and textContent, or sanitize the content with a trusted HTML sanitizer.
//textContent is safer for plain text because it does not interpret the value as HTML.
